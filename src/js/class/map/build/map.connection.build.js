...
export default class {
  ...
  create(group) {
    ...
    positionGroup.position.set(
      CHILD_PARAM.width / -2,
      CHILD_PARAM.height / 2 + CHILD_PARAM.y,
      this.param.z
    );
  ...
  createEffectGeometry() {
    const position = [
      ...new THREE.CircleGeometry(
        this.param.radius,
        32
      ).attributes.position.array.slice(3),
    ];
  ...
  initTween() {
    ...
    lines.children.forEach((line, idx) =>
      this.createLineTween({
        line,
        circle: circles.children[idx],
        effect: effects.children[idx],
        idx,
      })
    );
  }
  ...
  createLineTween({ line, circle, effect, idx }) {
    ...
    const circleProp = METHOD.getCircleProp({
      coords: this.map,
      ...CHILD_PARAM,
    });
    ...
    const tw1 = new TWEEN.Tween(start1)
      ...
      .onComplete(() =>
        this.completeLineTween1({ line, circle, effect, ...circleProp })
      );
