...
export default class {
  ...
  create(group) {
    ...
    for (let i = 0; i < this.param.count; i++) {
      ...
      localGroup.position.set(
        Math.random() * this.param.width - this.param.width / 2,
        Math.random() * this.param.height - this.param.height / 2,
        0
      );
  ...
  createLineGeometry({ x, y } = {}) {
    ...
    const { position, draw } = METHOD.createLineAttribute({
      x,
      y,
      ...this.param,
    });
  ...
  createPlaneGeometry() {
    return new THREE.PlaneGeometry(
      this.param.planeWidth,
      this.param.planeHeight
    );
  }
  createPlaneMaterial(idx) {
    this.ctx[idx] = METHOD.createCanvasTexture({
  ...
  updateOpenTween(moveGroup, { opacity }) {
    moveGroup.children.forEach(
      (child, i) =>
        (child.material.opacity = opacity * this.param.moveGroupOpacity[i])
    );
  }
  ...
  createMoveTween(line, moveGroup, route, idx) {
    ...
    this.tw[idx] = new TWEEN.Tween(start)
      .to(end, 70000 + dist * 2)
      .onUpdate(() =>
        this.updateMoveTween({ line, moveGroup, dist, start, x, y, idx })
      )
      .start();
  }
